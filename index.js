...
// ========================================================================================
// ↓↓↓ ВИДАЛИТИ ВЕСЬ ЦЕЙ БЛОК ПІСЛЯ ТОГО, ЯК БОТ ВІДПРАВИТЬ ПОВІДОМЛЕННЯ В ЧАТИ ↓↓↓
// ========================================================================================
...
* 📸 **Скріншоти більше не потрібні:** Для всіх **нових** тренувань я сам кидатиму інфу. Але для тих подій, що вже були в календарі до моєї появи, поки що скидайте скріни самі.
* 🚀 **Нові тренування:** Щойно тренування буде додано в Google Календар — воно миттєво з'явиться тут.
* 🔄 **Авто-оновлення:** Якщо час, майданчик чи склад учасників події змінилися, я **сам виправлю** повідомлення в чаті. Актуальна інфа завжди в останньому пості.
* ❌ **Скасування:** якщо тренування видалять з календаря — я одразу попереджу.
* 🔗 **Все під рукою:** У повідомленні завжди є посилання на подію в Google Календар.
...
Всі на розминку! 🏁`;
    ...
    try {
        ...
        for (const row of result.rows) {
            await bot.telegram.sendMessage(row.chat_id, welcomeText, { parse_mode: 'Markdown' });
            console.log(`Повідомлення надіслано в чат: ${row.chat_id}`);
        }
    } catch (err) {
